...
func main() {
	...
	textPath := flag.String("text", "largefile.txt", "Ruta del archivo de texto grande (UTF-8)")
	textIters := flag.Int("text-iters", 1_000_000, "Veces que se escribe una línea")
	textRepeats := flag.Int("text-repeats", 2, "Repeticiones del texto base por línea")

	binPath := flag.String("bin", "testfile.bin", "Ruta del archivo binario aleatorio")
	binBytes := flag.Int64("bin-bytes", 10*1024*1024, "Tamaño del archivo binario (bytes)")
	...
	// 3) testfile.bin
	fmt.Printf("Generando %s con %d bytes aleatorios...\n", *binPath, *binBytes)
	if err := genRandomBin(*binPath, *binBytes); err != nil {
		fatal(fmt.Errorf("falló generar %s: %w", *binPath, err))
	}
	fmt.Println("OK.")

	fmt.Println("Listo. Archivos generados:")
	fmt.Printf(" - %s\n - %s\n - %s\n", *numbersPath, *textPath, *binPath)
}
...
func validateRanges(minNum, maxNum, lines, textIters, textRepeats int, binBytes int64) error {
	...
	if binBytes <= 0 {
		return errors.New("bin-bytes debe ser > 0")
	}
	return nil
}
...
func genRandomBin(path string, totalBytes int64) error {
	f, err := os.Create(path)
	if err != nil {
		return err
	}
	...
	// Escribimos en chunks para no usar demasiada memoria
	...
	for written < totalBytes {
		toWrite := chunkSize
		if remaining := totalBytes - written; remaining < int64(chunkSize) {
			toWrite = int(remaining)
		}
		if _, err := io.ReadFull(cryptoRand.Reader, buf[:toWrite]); err != nil {
			return err
		}
